...
**Демо: https://sun-protector.vercel.app** — чат-бот работает в браузере и на телефоне
(если сайт долго не открывали, первый ответ может занять на несколько секунд дольше).
На публичном сайте доля тени иногда не считается: бесплатный сервер данных о домах
(Overpass) часто отвечает 504 на запросы с общих адресов Vercel — тогда маршрут
показывается без тени. Локально (`docker compose up`) тень считается стабильно.
...
Сделано как финальный проект курса. Полное техническое обоснование — в [ARCHITECTURE.md](ARCHITECTURE.md);
методология и результаты evals — в [EVALS.md](EVALS.md).

![Вечер: UV нет, маршрут на мини-карте](docs/screenshot-chat-night.png)
![День (центр Чикаго): тень по контурам и высоте домов — 56% против 35%](docs/screenshot-chat-day.png)

## Что делает

- **Чат**: всё общение — через переписку. «Можно выйти?», «проложи путь на
  работу», «какой крем нужен?» — агент сам решает, что вызвать (workflow
  LangGraph, RAG, геокодинг) и отвечает коротко; маршрут рисуется мини-картой
  прямо в сообщении. Тип кожи, время нанесения крема, адреса дома и работы
  запоминаются из переписки («у меня светлая кожа», «я живу на …»).
- **День и ночь**: время и восход/закат берутся по часовому поясу точки
  (Open-Meteo). Ночью UV нет — ассистент прямо говорит, что выходить безопасно.
...
- **Срочное предупреждение**: при очень высоком/экстремальном риске
  workflow уходит в отдельную ветку и первым делом говорит «уйдите в тень».

## Чек-лист требований курса

| Требование | Где |
|---|---|
| Многошаговый workflow LangGraph (ветвление, цикл) | `agent-service/app/graph/` |
...
| Веб-фронтенд (чат-бот) | `frontend/` (один `index.html`, без сборки); чат-агент — `agent-service/app/chat.py` |
...
Приложение опубликовано на Vercel как Docker-образ (`Dockerfile.vercel`, на
Vercel отправляются только папки из `.vercelignore`). Ключи хранятся в
защищённых переменных окружения проекта Vercel, не в репозитории.
...
API Open-Meteo/OSRM/OpenStreetMap); полный LangGraph workflow, включая ветвление
и цикл пересчёта; RAG-retrieval с корректно
...
маршрута, чат) — скриншоты выше сделаны в той самой сессии,
а не нарисованы вручную.

## Известные ограничения

- Тень считается упрощённо: по контурам и высоте домов из OpenStreetMap и
  позиции солнца, на плоской земле, без деревьев и навесов. У многих домов в
  OSM нет высоты — тогда берётся типовая (см. `mcp-server/tools/shade_route.py`).
- Формула риска использует общепринятые дерматологические учебные ориентиры,
  ...
  диагностически.
- Нет auth/ролей и нет CI-пайплайна — вне рамок
