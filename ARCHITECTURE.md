...
- Тень в `get_shade_route` — упрощённый расчёт на плоскости: из каждой точки
  маршрута «луч» к солнцу (позиция по формуле NOAA); точка в тени, если луч
  упирается в контур дома из OpenStreetMap ближе длины его тени
  (высота ÷ tan высоты солнца). Высота — из тегов `height` или
  `building:levels` × 3 м; если их нет (в Алматы примерно у половины домов) —
  по типу здания (дом ~6 м, гараж/киоск ~3 м, прочее ~9 м). Не учитываются
  деревья, навесы, рельеф и форма крыш. Публичный Overpass часто отвечает 504 —
  тогда берём центры домов вместо контуров, а успешные ответы кэшируем на
  сутки. Проверки геометрии — `mcp-server/tests/test_shade.py`.
...
- Если основная модель (Claude) недоступна по любой причине, `call_with_fallback()`
  ...
  баланс, а затем — когда Claude Sonnet 5 отверг параметр `temperature`.
  Урок: fallback прячет сбой основной
  модели от разработчика, поэтому смотреть на `model_used` / трейсы LangSmith.
